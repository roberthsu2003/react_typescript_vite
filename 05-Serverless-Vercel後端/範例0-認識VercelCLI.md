...
4. **親眼見證外洩 (二)：開發環境的 DevTools**
   ...
   按下 `Ctrl + Shift + F`（Windows）或 `Cmd + Option + F`（Mac）開啟全域搜尋，輸入你的 `AIza` 前綴金鑰。你會發現就算是開發模式載入的檔案，也能輕易搜出你的 API Key！
   
   這意味著未來只要將前端部署上線，任何人只要按下 F12 打開瀏覽器的開發者工具，就能輕易盜走你的 Key，並用你的帳號額度狂打 API！
