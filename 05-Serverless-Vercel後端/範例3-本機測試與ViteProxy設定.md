...
> **⚠️ 為什麼不使用舊版的 `builds` 設定？**
> 在早期的教學中，你可能會看到 `vercel.json` 裡寫了 `builds` 設定（例如指定 `@vercel/static-build` 和 `@vercel/node`）。
> **在現代的 Vercel 環境中，強烈建議避免這種舊寫法！**
> 
> 1. **零配置 (Zero Configuration)**：Vercel 現在非常聰明，會自動偵測 Vite 專案並執行 `npm run build`，也會自動處理 `/api` 資料夾內的 Serverless Functions，不需再手動指定打包規則。
> 2. **避免衝突**：強制加入 `builds` 會覆蓋掉 Vercel 的自動偵測機制，一旦加上反而常會導致「重複構建 (Double builds)」或產生未知錯誤。
> 
> 因此，我們現在只需定義 `rewrites` 去處理客戶端與 API 的路由分配，把編譯工作大膽交給 Vercel 自動化處理即可！
