...
講義中的程式碼與本 repo 的 [`tsx-cli/src/03-interfaces.ts`](tsx-cli/src/03-interfaces.ts) 僅供**對照**。**請在你自己的練習檔**中依下方步驟**手打或從講義複製**程式後再執行，**不要**只執行教材現成檔案。

---

## 步驟 0a：指令列練習（建議先做）

1. 開啟 [`tsx-cli/src/03-interfaces.ts`](tsx-cli/src/03-interfaces.ts)，**清空或覆寫**後依下方**步驟 1～6** 撰寫程式。
